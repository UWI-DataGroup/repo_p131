** HEADER -----------------------------------------------------
**  DO-FILE METADATA
    //  algorithm name			    bnr_survival_2013.do
    //  project:				        BNR
    ...
    ** Close any open log file and open a new log file
    capture log close
    log using "`logpath'\bnr_survival_2013", replace
** HEADER -----------------------------------------------------


* ************************************************************************
* SURVIVAL ANALYSIS
* Survival analysis to 4 years
**************************************************************************

** Load the dataset
use "`datapath'\version01\3-output\datarequest_NAACCR-IACR_matched_2013", clear
...
recode deceased 2=0 //381 changes
...
** JC (14feb2019) count if dod==. //381 so 450 are deceased
...
** set study end date variable as 4 years from dx date IF PT HAS NOT DIED
** JC (14feb2019): I used 4 yrs instead of 5 as we have death info up to end of 2017
gen end_date=(dot+(365.25*4)) if dot!=.
...
// that all end_dates are in between 2013-2017

** none with dod in 2018 so no need to reset to "alive". However,
** and any with dod >4 years from dx even if dod still in 2017,
** needs to be reset as alive
** JC (14feb2019) list deceased dot dod dlc if dod!=. & dod>dot+(365.25*4)
replace deceased=0 if dod!=. & dod>dot+(365.25*4) //7 changes

** set to missing those who have dod>4 years from incidence date - but
** first create new variable for time to death/date last seen, called "time"

** (1) use dod to define time to death if died within 4 yrs
gen time=dod-dot if (dod!=. & deceased==1 & dod<dot+(365.25*4))

** (2) next use 4 yrs as time, if died >4 yrs from incidence
...
replace time=dlc-dot if (dlc<end_date & deceased==0) //380 changes

** IRH (12feb2019) tab time ,m
** IRH (12feb2019) count if time!=. // at this point we are missing 9 for time... why?
** JC (14feb2019) count if time==. //8 missing time
...
replace time=dlc-dot if deceased==0 & time==. //1 change
...
** these are the 7 from above - change dod to missing (deceased already
** set to 0 above) as they did not die within 4 years
replace dod=. if end_date<dod & dod!=. //7 changes

** JC (14feb2019) tab deceased ,m // now 443 (used to be 450 but 7 died >4 years)
...
replace newend_date=dlc if (dlc<end_date) & dod==. & deceased==0 //380 changes
** JC (14feb2019) count if newend_date==. //8
** JC (14feb2019) list dot deceased dod dlc end_date if newend_date==.
replace newend_date=end_date if newend_date==. //8 changes
...
** there are 197 records with time=0 (ie either DCO or defaulted as not seen after dx date)
...
replace newend_date=newend_date+1 if (time==0 & deceased==0) //121 changes
replace time=1 if (time==0 & deceased==0) //121 changes
